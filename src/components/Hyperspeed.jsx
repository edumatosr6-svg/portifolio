  ...
  useEffect(() => {
    ...
    const distortions = {
      ...
      turbulentDistortion: {
        ...
        getJS: (progress, time) => {
          const { x, y, z, w } = turbulentUniforms.uFreq.value;
          const { x: ax, y: ay, z: az, w: aw } = turbulentUniforms.uAmp.value;
          const getX = p => Math.cos(Math.PI * p * x + time) * ax + Math.pow(Math.cos(Math.PI * p * y + time * (y / x)), 2) * ay;
          const getY = p => -nsin(Math.PI * p * z + time) * az - Math.pow(nsin(Math.PI * p * w + time / (z / w)), 5) * aw;
      ...
      deepDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = p => Math.sin(p * Math.PI * fx + time) * ax;
          const getY = p => Math.pow(p * py1, py2) + Math.sin(p * Math.PI * fy + time) * ay;
