...
const PillNav = ({
  ...
  baseColor = '#0d0d1b',
  pillColor = '#6366f1',
  hoveredPillTextColor = '#0d0d1b',
  pillTextColor,
  onMobileMenuClick,
  initialLoadAnimation = true
}) => {
  const resolvedPillTextColor = pillTextColor ?? '#f1f5f9';
  ...
  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        ...
        >
          {logo ? <img src={logo} alt={logoAlt} /> : 'E'}
        </a>
            ...
            {items.map((item, i) => {
              ...
              return (
                <li key={item.href || `item-${i}`} role="none">
                  {isInternal ? (
                    <Link role="menuitem" to={item.href} className={pillClass} aria-label={item.ariaLabel || item.label} onMouseEnter={() => handleEnter(i)} onMouseLeave={() => handleLeave(i)}>
                      {pillContent}
                    </Link>
                  ) : (
                    <a role="menuitem" href={item.href} target="_blank" rel="noopener noreferrer" className={pillClass} aria-label={item.ariaLabel || item.label} onMouseEnter={() => handleEnter(i)} onMouseLeave={() => handleLeave(i)}>
                      {pillContent}
                    </a>
                  )}
                </li>
      ...
      <div className="mobile-menu-popover mobile-only" ref={mobileMenuRef} style={cssVars}>
        <ul className="mobile-menu-list">
          {items.map((item, i) => {
            const isInternal = item.href?.startsWith('/');
            const cls = `mobile-menu-link${activeHref === item.href ? ' is-active' : ''}`;
            return (
              <li key={item.href || `mobile-item-${i}`}>
                {isInternal ? (
                  <Link to={item.href} className={cls} onClick={() => setIsMobileMenuOpen(false)}>{item.label}</Link>
                ) : (
                  <a href={item.href} target="_blank" rel="noopener noreferrer" className={cls} onClick={() => setIsMobileMenuOpen(false)}>{item.label}</a>
                )}
              </li>
            );
          })}
